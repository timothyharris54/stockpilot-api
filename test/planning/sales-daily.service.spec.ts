...
describe('SalesDailyService', () => {
    ...
    beforeEach(async () => {
        ...
        planningSettingsService = module.get<PlanningSettingsService>(PlanningSettingsService);
    ...
    it('throws when no demand order statuses are configured', async () => {
        jest.spyOn(service['planningSettingsService'], 'getDemandOrderStatuses').mockResolvedValue([]);
        await expect(
            service.rebuildForAccount(
            1n,
            new Date('2026-03-20T00:00:00.000Z'),
    ...
    it('uses demand statuses from planning settings', async () => {
        planningSettingsServiceMock.getDemandOrderStatuses.mockResolvedValue(['completed']);
        planningSettingsServiceMock.getDemandDateBasis.mockResolvedValue(DEFAULT_DEMAND_DATE_BASIS);
        ...
        expect(planningSettingsServiceMock.getDemandOrderStatuses).toHaveBeenCalledWith(1n);
    });
