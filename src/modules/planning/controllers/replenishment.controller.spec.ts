...
describe('ReplenishmentController', () => {
  ...
  it('calls generateForProduct with identity.accountId and productId', async () => {
    ...
    const result = await controller.generateForProduct(
      identity,
      '2',
      'MAIN',
      'true',
    ...
    expect(
      replenishmentEngineServiceMock.generateForProduct,
    ).toHaveBeenCalledWith(1n, 2n, 'MAIN', true);
  ...
  it('calls generateForAccount with accountId', async () => {
    ...
    const result = await controller.generateForAccount(
      identity,
      'MAIN',
      'true',
    );
    ...
    ).toHaveBeenCalledWith(1n, 'MAIN', true);
    expect(result).toEqual([{ ok: true }]);
  });
});
